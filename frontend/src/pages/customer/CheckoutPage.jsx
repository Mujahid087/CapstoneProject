...
const labelIcons = { Home: "Home", Office: "Office", Other: "Other" };
            ...
            <Card.Body>
              ...
              <div className="table-responsive">
                <table className="table table-sm mb-0">
                  ...
                  <tbody>
                    {items.map((item, idx) => (
                      <tr key={item.cartKey || item.itemId || idx}>
                        <td>
                          <div>{item.name}</div>
                          <div className="text-muted small">{formatCustomization(item.customization)}</div>
                        </td>
                        <td className="text-center">{item.quantity || 1}</td>
                        <td className="text-end">Rs.{(item.price || 0) * (item.quantity || 1)}</td>
                      </tr>
                    ))}
                  </tbody>
                  <tfoot>
                    <tr className="fw-bold">
                      ...
                      <td className="text-end text-muted">Rs.{itemsTotal}</td>
                    </tr>
                    {discountAmount > 0 && (
                      <tr className="fw-bold text-success">
                        <td colSpan="2">Discount (5%)</td>
                        <td className="text-end">-Rs.{discountAmount}</td>
                      </tr>
                    )}
                    <tr className="fw-bold">
                      <td colSpan="2">Delivery Fee</td>
                      <td className="text-end text-muted">Rs.{deliveryFee}</td>
                    </tr>
                    <tr className="fw-bold">
                      <td colSpan="2">Tax (5%)</td>
                      <td className="text-end text-muted">Rs.{tax}</td>
                    </tr>
                    <tr className="fw-bold">
                      <td colSpan="2">Final Total</td>
                      <td className="text-end fs-5 text-danger">Rs.{totalAmount}</td>
                    </tr>
                  </tfoot>
                </table>
              </div>

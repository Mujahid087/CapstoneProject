...
function OrdersPage() {
    ...
    const subtotal = bill?.items?.reduce((sum, item) => sum + (item.price || 0) * (item.quantity || 1), 0) || 0;
        ...
        <>
            ...
            {orders.length === 0 ? (
                <Card className="shadow-sm border-0 text-center py-5">
                    <Card.Body>
                        <div className="fs-1 mb-3">Pizza</div>
                        ...
                        <tbody>
                            {paginatedOrders.map((order) => {
                                ...
                                const getOrderTotal = () => {
                                    ...
                                    const currentDeliveryFee = order.deliveryMode === "delivery" ? 50 : 0;
                                    const currentTax = Math.round(itemsTotal * 0.05);
                                    const discountAmount = order.discountAmount || 0;

                                    return itemsTotal - discountAmount + currentDeliveryFee + currentTax;
                                };
                                ...
                                return (
                                    <tr key={order._id}>
                                        ...
                                        <td className="text-end fw-bold">{formatCurrency(finalAmount)}</td>
                            ...
                            <div className="p-4">
                                ...
                                <Row className="mb-3">
                                    <Col sm={6}>
                                        ...
                                        {bill.phone && <div className="small text-muted">Phone: {bill.phone}</div>}
                                    </Col>
                                    <Col sm={6}>
                                ...
                                <div className="border rounded mt-3 p-3">
                                    ...
                                    <div className="d-flex justify-content-between mb-1">
                                        <span className="text-muted">
                                            Delivery Fee{bill.deliveryMode === "pickup" ? " (Pickup)" : ""}
                                        </span>
                                    ...
                                    <div className="d-flex justify-content-between">
                                        ...
                                        <strong className="fs-5 text-danger">
                                            {formatCurrency(subtotal + deliveryFee + tax)}
                                        </strong>
                                    </div>
                                </div>
